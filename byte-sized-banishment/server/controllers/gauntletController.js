...
export const startGauntlet = async (req, res) => {
    ...
    try {
        ...
        const mockLastQuestion = { difficulty: difficulty === "easy" ? "easy" : "medium" };
        const firstQuestion = await selectNextQuestion(session, mockLastQuestion, true);
        if (!firstQuestion) return res.status(404).json({ message: `No questions found for subject: ${subject}` });
    ...
    try {
        ...
        if (isCorrect) {
            ...
            if (user.activeEffect && user.activeEffect.type && user.activeEffect.expiresAt > new Date()) {
                ...
                user.activeEffect = { type: null, name: null, modifier: 1, expiresAt: null };
            }
            ...
            if (user.xp >= user.xpToNextLevel) {
                 ...
                 devilDialogue = { text: `You've reached Level ${user.level}! Your new rank is ${user.rank}. Don't get cocky.` };
            }
        } else {
            ...
            if (session.correctStreak === 0 && session.strikesLeft === 1 && !user.activeEffect.type) {
                ...
                devilDialogue = { text: "You're faltering. You've been cursed with Crippling Doubt! Your XP gains are halved for 5 minutes." };
            }
        }
        ...
        if (session.strikesLeft <= 0) {
            ...
            return res.json({
                ...
                isGameOver: true,
                punishment: { type: 'penance', task: randomPenance.task, quote: randomPenance.quote }
            });
        }
